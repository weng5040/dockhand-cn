<script lang="ts">
	...
	function connect() {
		...
		terminal.writeln(`\x1b[90mShell: ${selectedShell}, 用户: ${selectedUser || 'default'}\x1b[0m`);
		terminal.writeln('');
			...
			<div class="flex-1 flex items-center justify-center p-6">
				...
				{:else if !anyShellAvailable}
					<div class="text-center">
						...
						<h3 class="text-lg font-medium text-amber-500">无可用 shell</h3>
						<p class="text-sm text-muted-foreground mt-2">
							此容器未安装任何 shell。
						</p>
						<p class="text-xs text-muted-foreground/70 mt-1">
							基于 scratch 或无发行版镜像构建的容器通常不包含 shell。
						</p>

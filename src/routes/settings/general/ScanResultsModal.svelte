	...
	async function handleAdopt() {
		...
		try {
			...
			for (const [envId, stacks] of stacksByEnv) {
				...
				if (!response.ok) {
					...
					for (const stack of stacks) {
						totalFailed.push({ name: stack.name, error: data.error || '采用失败' });
					}
				} else {
			...
			if (totalAdopted.length > 0) {
				toast.success(`已采用 ${totalAdopted.length} 个应用栈`);
			}
			if (totalFailed.length > 0) {
				toast.error(`采用 ${totalFailed.length} 个应用栈失败`);
			}
...
<Dialog.Root bind:open onOpenChange={(isOpen) => !isOpen && onclose()}>
	<Dialog.Content class="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
		<Dialog.Header>
			...
			<Dialog.Description>
				已扫描 {scannedPaths.length} 个配置的路径{scannedPaths.length !== 1 ? '' : ''}，查找 Docker Compose 文件
			</Dialog.Description>
		</Dialog.Header>
			...
			<div class="space-y-3 py-2 border-b bg-background">
				...
				<div class="flex items-center gap-4 p-3 rounded-lg bg-muted/50">
					<div class="flex items-center gap-2">
						...
						<span class="text-sm font-medium">{result.discovered.length + result.skipped.length + adoptedStacks.length} 个已发现</span>
					</div>
					{#if result.discovered.length > 0}
						<div class="flex items-center gap-1.5 text-blue-600 dark:text-blue-500">
							<Import class="w-4 h-4" />
							<span class="text-sm">{result.discovered.length} 个新应用栈</span>
						</div>
					{/if}
					{#if adoptedStacks.length > 0}
						<div class="flex items-center gap-1.5 text-green-600 dark:text-green-500">
							<CheckCircle2 class="w-4 h-4" />
							<span class="text-sm">{adoptedStacks.length} 个已采用</span>
						</div>
					{/if}
					{#if result.skipped.length > 0}
						<div class="flex items-center gap-1.5 text-muted-foreground">
							<SkipForward class="w-4 h-4" />
							<span class="text-sm">{result.skipped.length} 个已采用</span>
						</div>
					{/if}
				...
				{#if result.discovered.length > 0}
					{#if loadingEnvs}
						<div class="flex items-center gap-3 p-3 rounded-lg border bg-muted/30">
							<Label class="text-sm font-medium shrink-0">采用到：</Label>
					...
					{:else if environments.length === 0}
						<div class="flex items-center gap-3 p-3 rounded-lg border bg-muted/30">
							<p class="text-sm text-destructive">未配置环境</p>
						</div>
												...
												<Badge variant="outline" class="text-xs text-green-600 dark:text-green-500 border-green-300 dark:border-green-600 gap-1">
													...
													<Tooltip.Root>
														...
														<Tooltip.Content class="max-w-sm">
															<p class="text-xs">此应用栈已在运行（通过 Docker 的 <code class="bg-muted px-1 rounded">com.docker.compose.project</code> 标签检测到）。采用后，您可以通过 Dockhand 管理它。</p>
														</Tooltip.Content>
													</Tooltip.Root>
												</Badge>
											{/if}
											...
											{#if stack.runningOn && stack.runningOn.length > 0 && !stack.runningOn.some(r => r.envId === stackEnvId)}
												<Badge variant="outline" class="text-xs text-amber-600 dark:text-amber-500 border-amber-300 dark:border-amber-600 gap-1">
													...
													<Tooltip.Root>
														...
														<Tooltip.Content class="max-w-sm">
															<p class="text-xs">此应用栈运行于其他环境 ({stack.runningOn?.map(r => r.envName).join(', ')})。您仍可以在这里采用它，但这不会影响正在运行的容器。</p>
														</Tooltip.Content>
													</Tooltip.Root>
												</Badge>
											{/if}
		...
		<Dialog.Footer class="flex items-center justify-between">
			...
			<div class="flex items-center gap-2">
				...
				{:else if adoptedStacks.length > 0}
					<Button href="/stacks">查看应用栈</Button>
				{/if}
			</div>
		</Dialog.Footer>
	</Dialog.Content>

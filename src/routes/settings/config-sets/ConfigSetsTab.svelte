...
<div class="space-y-4">
			...
			<div class="flex items-start gap-3">
				...
				<div>
					...
					<p class="text-xs text-muted-foreground mt-1">
						配置集是可重用的容器配置模板。定义一次常用的环境变量、标签、端口和数据卷，然后在创建或编辑容器时应用它们。配置集中的值可以在容器创建期间被覆盖。
					</p>
				</div>
			</div>
		</Card.Content>
	...
	<div class="flex justify-between items-center">
		<div class="flex items-center gap-3">
			<Badge variant="secondary" class="text-xs">{configSets.length} 个总计</Badge>
		</div>
				...
				<Card.Root>
					...
					<Card.Content class="space-y-3">
						...
						<div class="text-xs text-muted-foreground">
							<span>网络: {cfg.networkMode}</span>
							<span class="mx-1">|</span>
							<span>重启: {cfg.restartPolicy}</span>
						</div>

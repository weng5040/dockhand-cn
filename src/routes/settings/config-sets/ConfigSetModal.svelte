		...
		<div class="space-y-4">
			...
			<div class="grid grid-cols-2 gap-4">
				...
				<div class="space-y-2">
					...
					<Input id="cfg-description" bind:value={formDescription} placeholder="Web服务的通用设置" />
				</div>
			</div>
			...
			<div class="grid grid-cols-2 gap-4">
				...
				<div class="space-y-2">
					...
					<Select.Root type="single" value={formRestartPolicy} onValueChange={(v) => formRestartPolicy = v}>
						<Select.Trigger class="w-full">
							<span>{formRestartPolicy === 'no' ? '不重启' : formRestartPolicy === 'always' ? '总是' : formRestartPolicy === 'on-failure' ? '失败时' : '除非停止'}</span>
						</Select.Trigger>
						<Select.Content>
			...
			<div class="space-y-2 border-t pt-4">
				...
				{#each formEnvVars as envVar, i}
					<div class="flex gap-2 items-center">
						<Input bind:value={envVar.key} placeholder="键" class="flex-1 h-8" />
						<Input bind:value={envVar.value} placeholder="值" class="flex-1 h-8" />
			...
			<div class="space-y-2 border-t pt-4">
				...
				{#each formLabels as label, i}
					<div class="flex gap-2 items-center">
						<Input bind:value={label.key} placeholder="标签.键" class="flex-1 h-8" />
						<Input bind:value={label.value} placeholder="值" class="flex-1 h-8" />
			...
			<div class="space-y-2 border-t pt-4">
				<div class="flex justify-between items-center">
					<Label class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">卷映射</Label>

<script lang="ts">
	...
	async function save() {
		...
		if (!formName.trim()) {
			formErrors.name = '名称必填';
			hasErrors = true;
		}
		if (!formIssuerUrl.trim()) {
			formErrors.issuerUrl = 'Issuer URL 必填';
			hasErrors = true;
		}
		if (!formClientId.trim()) {
			formErrors.clientId = '客户端 ID 必填';
			hasErrors = true;
		}
		if (!isEditing && !formClientSecret.trim()) {
			formErrors.clientSecret = '客户端密钥必填';
			hasErrors = true;
		}
		if (!formRedirectUri.trim()) {
			formErrors.redirectUri = '重定向 URI 必填';
			hasErrors = true;
		}
		...
		try {
			...
			if (response.ok) {
				...
				formError = data.error || `无法${isEditing ? '更新' : '创建'} OIDC 配置`;
			}
		} catch {
			formError = `无法${isEditing ? '更新' : '创建'} OIDC 配置`;
		} finally {
			formSaving = false;
		}
	}
	...
	<Dialog.Content class="max-w-2xl h-[80vh] flex flex-col overflow-hidden">
		...
		<Tabs.Root bind:value={formActiveTab} class="flex-1 flex flex-col overflow-hidden">
			<Tabs.List class="flex-shrink-0 grid w-full grid-cols-2">
				<Tabs.Trigger value="general">常规</Tabs.Trigger>
			...
			<Tabs.Content value="general" class="flex-1 overflow-y-auto space-y-4 py-2 mt-0">
				...
				<div class="space-y-4">
					...
					<p class="text-xs text-muted-foreground">从身份提供商的 application settings 获取这些信息。</p>
					<div class="grid grid-cols-2 gap-4">
						<div class="space-y-2">
						...
						<div class="space-y-2">
							...
							<Input
								...
								placeholder={isEditing ? '留空以保持现有值' : 'your-client-secret'}
				...
				<div class="space-y-4">
					...
					<div class="space-y-2">
						...
						{:else}
							<p class="text-xs text-muted-foreground">将此 URI 添加到身份提供商的允许回调 URL 列表中。</p>
						{/if}
					</div>
					<div class="space-y-2">
						<Label>作用域</Label>
			...
			<Tabs.Content value="role-mapping" class="flex-1 overflow-y-auto space-y-4 py-2 mt-0">
					...
					<div class="flex-1 flex items-center justify-center py-8">
						<div class="text-center">
							...
							<p class="text-sm text-muted-foreground mb-4 max-w-md mx-auto">
								角色映射允许您根据身份提供商的组或声明自动分配 Dockhand 角色。此功能需要企业版许可证。
							</p>
				...
				{:else}
					...
					<div class="space-y-4">
						<h4 class="text-sm font-medium text-muted-foreground">组/角色声明</h4>
					...
					<div class="space-y-4">
						<div class="flex items-center justify-between">
							<div>
								...
								<p class="text-xs text-muted-foreground mt-0.5">将身份提供商的声明值映射到 Dockhand 角色。</p>
							</div>
							<Button
								size="sm"
								variant="outline"
								...
								{#each formRoleMappings as mapping, index}
									<div class="flex items-center gap-3 p-3 bg-muted/50 rounded-lg">
										<div class="flex-1 grid grid-cols-2 gap-3">
											...
											<div class="space-y-1">
												<Label class="text-xs">Dockhand 角色</Label>
												<Select.Root
													type="single"

<script lang="ts">
	...
	async function save() {
		...
		if (!formServerUrl.trim()) {
			formErrors.serverUrl = '服务器 URL 必填';
			hasErrors = true;
		}
		if (!formBaseDn.trim()) {
			formErrors.baseDn = 'Base DN 必填';
			hasErrors = true;
		}
		...
		try {
			...
			if (response.ok) {
				...
				formError = data.error || `LDAP 配置${isEditing ? '更新' : '创建'}失败`;
			}
		} catch {
			formError = `LDAP 配置${isEditing ? '更新' : '创建'}失败`;
		} finally {
			formSaving = false;
		}
	}
		...
		<div class="flex-1 overflow-y-auto py-2">
			...
			<Tabs.Root bind:value={formModalTab}>
				<Tabs.List class="grid w-full grid-cols-2 mb-4">
					...
					<Tabs.Trigger value="groups">组设置</Tabs.Trigger>
				</Tabs.List>
				...
				<Tabs.Content value="connection" class="space-y-4">
					...
					<div class="space-y-4">
						...
						<p class="text-xs text-muted-foreground">用于搜索用户的服务账户。留空表示匿名绑定。</p>
						<div class="grid grid-cols-2 gap-4">
							<div class="space-y-2">
								<Label>绑定 DN</Label>
								<Input
							...
							<div class="space-y-2">
								...
								<Input
									...
									placeholder={isEditing ? '留空以保持现有密码' : '输入密码'}
								/>
							</div>
						</div>
					</div>
					...
					<div class="space-y-4">
						...
						<div class="space-y-2">
							...
							{:else}
								<p class="text-xs text-muted-foreground">搜索用户的基础 DN。</p>
							{/if}
						</div>
						<div class="space-y-2">
							<Label>用户过滤器</Label>
				...
				<Tabs.Content value="groups" class="space-y-4">
					<!-- Group Settings -->
					<div class="space-y-4">
						<h4 class="text-sm font-medium text-muted-foreground">组设置</h4>
						<p class="text-xs text-muted-foreground">配置基于组的访问控制。这些设置是可选的。</p>
						<div class="grid grid-cols-2 gap-4">
							<div class="space-y-2">
								<Label>组基础 DN</Label>
								<Input
								...
								/>
								<p class="text-xs text-muted-foreground">搜索组的基础 DN。</p>
							</div>
							<div class="space-y-2">
								<Label>管理员组</Label>
								<Input
								...
								/>
								<p class="text-xs text-muted-foreground">此组的成员将成为管理员。</p>
							</div>
						</div>
						<div class="space-y-2">
							<Label>成员过滤器</Label>
							...
							/>
							<p class="text-xs text-muted-foreground">
								查找用户所属组的过滤器。使用 <code class="text-xs bg-muted px-1 rounded">{'{{user_dn}}'}</code> 作为占位符。
							</p>
						</div>
					</div>
					...
					{#if isEnterprise}
						<div class="space-y-4">
							<div class="flex items-center gap-2">
								<h4 class="text-sm font-medium text-muted-foreground">组到角色映射</h4>

...
<div class="flex items-start gap-3">
	<div class="flex-1">
		<Label>收集系统指标</Label>
		<p class="text-xs text-muted-foreground">从此环境中收集CPU和内存使用统计信息</p>
	</div>
...
<div class="flex items-start gap-3">
	<div class="flex-1">
		<Label>高亮值变化</Label>
		<p class="text-xs text-muted-foreground">当容器列表中的容器值发生变化时显示琥珀色光晕</p>
	</div>
...
<div class="border-t pt-4 mt-2 space-y-3">
	<div class="flex items-start gap-3">
		<div class="flex-1">
			<Label>磁盘空间警告</Label>
			<p class="text-xs text-muted-foreground">当Docker磁盘使用量超过阈值时发送通知</p>
		</div>

	...
	async function save() {
		...
		try {
			...
			if (response.ok) {
				...
				formError = data.error || `镜像仓库${isEditing ? '更新' : '创建'}失败`;
			}
		} catch {
			formError = `镜像仓库${isEditing ? '更新' : '创建'}失败`;
		} finally {
			formSaving = false;
		}
	}
	...
	<Dialog.Content class="max-w-md">
		...
		<div class="space-y-4">
			...
			<div class="space-y-2">
				...
				<Input id="reg-name" bind:value={formName} placeholder="我的私有镜像仓库" />
			</div>
			<div class="space-y-2">
			...
			<div class="space-y-4 pt-2 border-t">
				<p class="text-xs text-muted-foreground">凭据 {isEditing ? '(密码留空以保持现有凭据)' : '(可选)'}</p>
				<div class="space-y-2">
				...
				<div class="space-y-2">
					...
					<Input id="reg-password" type="password" bind:value={formPassword} placeholder={isEditing ? '留空以保持现有凭据' : '密码或访问令牌'} />
				</div>
			</div>
		</div>
		<Dialog.Footer>

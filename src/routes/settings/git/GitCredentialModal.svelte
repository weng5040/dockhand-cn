...
<Dialog.Root bind:open onOpenChange={(o) => { if (o) focusFirstInput(); else onClose(); }}>
	<Dialog.Content class="max-w-md">
		<Dialog.Header>
			...
			<Dialog.Description>
				{isEditing ? '更新凭据设置' : '创建新凭据以访问 Git 仓库'}
			</Dialog.Description>
		</Dialog.Header>
		...
		<form onsubmit={(e) => { e.preventDefault(); saveCredential(); }} class="space-y-4">
			<div class="space-y-2">
				...
				{:else if !isEditing}
					<p class="text-xs text-muted-foreground">用于标识此凭据的友好名称</p>
				{/if}
			</div>
				...
				{#if formAuthType === 'password'}
					...
					<div class="space-y-2">
						...
						<Input
							...
							placeholder={isEditing ? '留空以保持当前' : '密码或个人访问令牌'}
						...
						{:else if isEditing && credential?.hasPassword}
							<p class="text-xs text-muted-foreground">当前密码已设置。留空以保持。</p>
						{/if}
					</div>
				{:else if formAuthType === 'ssh'}
					<div class="space-y-2">
						...
						{:else if isEditing && credential?.hasSshKey}
							<p class="text-xs text-muted-foreground">当前 SSH 密钥已设置。留空以保持。</p>
						{/if}
					</div>
					<div class="space-y-2">
						<Label for="cred-ssh-passphrase">SSH 密码短语（可选）</Label>
						<Input id="cred-ssh-passphrase" type="password" bind:value={formSshPassphrase} placeholder="加密密钥的密码短语" />
					</div>
				{/if}
			</div>

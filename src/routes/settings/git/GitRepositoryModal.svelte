<script lang="ts">
	...
	async function saveRepository() {
		...
		if (!formName.trim()) {
			formErrors.name = '名称是必需的';
		}

		if (!formUrl.trim()) {
			formErrors.url = '仓库 URL 是必需的';
		}
...
<Dialog.Root bind:open onOpenChange={(o) => { if (o) focusFirstInput(); else onClose(); }}>
	<Dialog.Content class="max-w-lg">
		<Dialog.Header>
			...
			<Dialog.Description>
				{isEditing ? '更新仓库设置' : '添加可用于部署应用栈的 Git 仓库'}
			</Dialog.Description>
		</Dialog.Header>
		...
		<form onsubmit={(e) => { e.preventDefault(); saveRepository(); }} class="space-y-4">
			<div class="space-y-2">
				...
				{:else if !isEditing}
					<p class="text-xs text-muted-foreground">用于标识此仓库的友好名称</p>
				{/if}
			</div>

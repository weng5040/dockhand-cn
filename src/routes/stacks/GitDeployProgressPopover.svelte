	...
	>
		...
		<div class="px-6 py-4 border-b shrink-0">
			<div class="flex items-center gap-2 min-w-0">
				...
				{#if overallStatus === 'complete'}
					<Badge variant="outline" class="ml-auto shrink-0 text-green-600 border-green-600/30">完成</Badge>
				{:else if overallStatus === 'error'}
					<Badge variant="outline" class="ml-auto shrink-0 text-red-600 border-red-600/30">失败</Badge>
				{:else if isDeploying}
				...
				<div class="flex items-start gap-3 py-2 px-2">
					...
					<div class="space-y-1">
						...
						<p class="text-sm text-muted-foreground">
							这将为 <strong class="text-foreground">{stackName}</strong> 拉取最新更改。
							仅当配置发生更改时，容器才会重启。
						</p>
					</div>
				</div>

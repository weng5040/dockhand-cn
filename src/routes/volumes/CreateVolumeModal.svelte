...
<script lang="ts">
	...
	const VOLUME_DRIVERS = [
		...
		{ value: 'cifs', label: 'CIFS', description: 'Windows/SMB共享', icon: Database }
	];
	...
	async function handleCreate() {
		...
		if (driver === 'cifs') {
			if (!cifsServer.trim()) errors.server = '服务器必填';
			if (!cifsShare.trim()) errors.share = '共享路径必填';
		} else if (driver === 'nfs') {
			if (!nfsServer.trim()) errors.server = '服务器必填';
			if (!nfsPath.trim()) errors.path = '导出路径必填';
		}
	...
	<Dialog.Content class="max-w-2xl">
		...
		<div class="space-y-4">
			...
			{#if driver === 'cifs'}
				...
				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-2">
						<Label for="cifs-version">SMB版本</Label>
					...
					<div class="space-y-2">
						...
						<Input
							...
							disabled={creating}
						/>
						<p class="text-xs text-muted-foreground">可选的AD/工作组域</p>
					</div>
				</div>
					...
					{#if showAdditionalOpts}
						<div class="space-y-2 pl-1">
							...
							{#if driverOpts.length > 0}
								...
								{/each}
							{:else}
								<p class="text-xs text-muted-foreground">额外的挂载选项将追加到挂载字符串</p>
							{/if}
						</div>
					{/if}
				</div>
			{:else if driver === 'nfs'}
				...
				<div class="space-y-2">
					<Label for="nfs-version">NFS版本</Label>
				...
				<div class="flex items-center gap-6">
					<div class="flex items-center gap-2">
						<TogglePill bind:checked={nfsSoft} onLabel="软" offLabel="硬" />
			...
			{:else}
				...
				<div class="space-y-2">
					...
					{:else}
						<p class="text-xs text-muted-foreground">未配置驱动选项</p>
					{/if}
				</div>
			{/if}
			...
			<div class="space-y-2">
				...
				{:else}
					<p class="text-xs text-muted-foreground">未配置标签</p>
				{/if}
			</div>

<script lang="ts">
	...
	const datePresets = [
		...
		{ value: 'last7days', label: '过去 7 天' },
		{ value: 'last30days', label: '过去 30 天' },
		...
		{#if $licenseStore.isEnterprise}
			<div class="flex flex-wrap items-center gap-2">
				...
				<span
					...
					title={$auditSseConnected ? '实时更新已启用' : '连接中...'}
				>
	...
	{:else if !$licenseStore.isEnterprise}
		<div class="flex flex-col items-center justify-center py-16 text-center">
			...
			<p class="text-muted-foreground max-w-md mb-6">
				审计日志是企业版功能，用于跟踪所有用户操作以确保合规性和安全监控。
			</p>
			...
			{#snippet footer()}
				...
				{:else if !hasMore && logs.length > 0}
					<div class="text-center py-2 text-sm text-muted-foreground">
						结果结束（共 {total.toLocaleString()} 条记录）
					</div>
				{/if}
			{/snippet}
		</DataGrid>
		...
		{#if selectedLog}
			<div class="space-y-4">
				...
				{#if selectedLog.details?.changes}
					<div>
						<label class="text-sm font-medium text-muted-foreground mb-2 block">变更</label>

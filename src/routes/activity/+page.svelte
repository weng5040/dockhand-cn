<script lang="ts">
	...
	const actionOptions = [
		...
		{ value: 'die', label: '异常退出', icon: Skull, color: 'text-red-500' },
		{ value: 'kill', label: '强制终止', icon: Zap, color: 'text-red-500' },
		...
		<div class="flex items-center gap-3">
			...
			<Badge variant="outline" class="gap-1.5 {($appSettings.eventCollectionMode || 'stream') === 'stream' ? 'text-green-500 border-green-500/50' : 'text-amber-500 border-amber-500/50'}">
				...
				{:else if ($appSettings.eventCollectionMode || 'stream') === 'poll'}
					<Radio class="w-3 h-3" />
					<span>轮询</span><span class="text-[10px] opacity-70">({($appSettings.eventPollInterval || 60000) / 1000}秒)</span>
				{:else}
		...
		>
			...
			{#snippet emptyState()}
				<div class="flex flex-col items-center justify-center py-16 text-muted-foreground">
					...
					<p>未找到容器事件</p>
					<p class="text-xs mt-1">容器启动、停止等事件将显示在这里。</p>
				</div>
			{/snippet}
			...
			{#snippet footer()}
				...
				{:else if !hasMore && events.length > 0}
					<div class="text-center py-2 text-sm text-muted-foreground">
						结果结束 (共{total.toLocaleString()}个事件)
					</div>
				{/if}
			{/snippet}
		</DataGrid>

<script lang="ts" module>
	...
	const NETWORK_DRIVERS = [
		{ value: 'bridge', label: 'Bridge', description: '默认网络驱动' },
		{ value: 'host', label: 'Host', description: '直接使用主机网络' },
		...
		{ value: 'none', label: 'None', description: '禁用网络' }
	] as const;

	const COMMON_DRIVER_OPTIONS: Record<string, { key: string; description: string }[]> = {
		bridge: [
			{ key: 'com.docker.network.bridge.name', description: '网桥设备名称' },
	...
	async function handleSubmit() {
		...
		if (needsParentConfig) {
			if (!parentInterface.trim()) {
				errors.parentInterface = `父接口对于 ${driver} 网络是必需的`;
				hasErrors = true;
			}
			if (!subnet.trim()) {
				errors.subnet = `子网对于 ${driver} 网络是必需的`;
				hasErrors = true;
			}
		}
	...
	<Dialog.Content class="max-w-3xl max-h-[90vh] overflow-y-auto">
		...
		<Tabs.Root value="basic" class="mt-4">
			...
			<div class="min-h-[200px] sm:min-h-[300px] mt-4">
				...
				{#if needsParentConfig}
					<div class="bg-amber-500/10 border border-amber-500/20 rounded-md p-3 space-y-3">
						<p class="text-xs font-medium text-amber-600 dark:text-amber-400">
							{driver === 'macvlan' ? 'Macvlan' : 'IPvlan'} 配置 (必需)
						</p>
								...
								<div class="space-y-1">
									...
									<Select.Root type="single" bind:value={macvlanMode}>
										<Select.Trigger class="h-8 text-xs">
											...
											<span>{macvlanMode === 'bridge' ? 'Bridge (默认)' : macvlanMode === 'private' ? 'Private' : macvlanMode === 'vepa' ? 'VEPA' : 'Passthru'}</span>
										</Select.Trigger>
										<Select.Content>
											<Select.Item value="bridge" label="Bridge (默认)">
												<Layers class="w-3 h-3 mr-1.5 text-muted-foreground" />Bridge (默认)
											</Select.Item>
				...
				<div class="space-y-3 pt-2">
					<div class="flex items-center gap-3">
						...
						<div>
							...
							<span class="text-muted-foreground text-xs block">限制对此网络的外部访问</span>
						</div>
					</div>
					...
					<div class="flex items-center gap-3">
						...
						<div>
							...
							<span class="text-muted-foreground text-xs block">允许手动附加容器 (overlay 网络)</span>
						</div>
					</div>
				...
				<div class="space-y-2">
					<Label for="ipRange">IP 范围</Label>
					<Input id="ipRange" bind:value={ipRange} placeholder="172.20.10.0/24" />
					<p class="text-xs text-muted-foreground">从子网的子范围分配容器 IP</p>
				</div>
				...
				<div class="space-y-2">
					...
					<p class="text-xs text-muted-foreground">设置驱动特定选项 (-o key=value)</p>

					{#if COMMON_DRIVER_OPTIONS[driver]?.length > 0}
						<div class="bg-muted/50 rounded-md p-3 text-xs space-y-1">
							<p class="font-medium">{driver} 驱动的常用选项:</p>
				...
				<div class="space-y-2">
					...
					<p class="text-xs text-muted-foreground">在网络设置元数据标签</p>
					...
					{/each}
					{#if labels.length === 0}
						<p class="text-xs text-muted-foreground italic">未配置标签</p>
					{/if}
				</div>
				</Tabs.Content>
			</div>
		...
		{#if errors.name || errors.parentInterface || errors.subnet}
			<Alert.Root variant="destructive" class="mt-4">
				<TriangleAlert class="h-4 w-4" />
				<Alert.Description>请修复上述验证错误</Alert.Description>
			</Alert.Root>
		{/if}

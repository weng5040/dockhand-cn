			...
			<div class="flex gap-2 items-center">
					...
					<div class="flex items-center h-9 w-5 justify-center shrink-0">
						{#if source === 'file'}
							<Tooltip.Root>
								...
								<Tooltip.Content side="bottom"><p class="whitespace-nowrap">来自仓库中的环境文件</p></Tooltip.Content>
							</Tooltip.Root>
						{:else if source === 'override'}
							<Tooltip.Root>
								...
								<Tooltip.Content side="bottom"><p class="whitespace-nowrap">{fileValues[variable.key] !== undefined ? '恢复为文件中的值' : '手动覆盖（不在文件中）'}</p></Tooltip.Content>
							</Tooltip.Root>
						{/if}
					</div>
				{/if}
						...
						{#if variable.key}
							{#if isVarRequired && !isVarMissing}
								<Tooltip.Root>
									...
									<Tooltip.Content><p>必需变量已定义</p></Tooltip.Content>
								</Tooltip.Root>
				...
				{#if !readonly}
					...
					{#if existingSecret}
						...
						<div class="flex items-center h-9 w-9 justify-center shrink-0" title="秘密值（无法取消隐藏）">
					...
					{:else}
						...
						<button
							...
							title={variable.isSecret ? '标记为秘密' : '标记为秘密'}
